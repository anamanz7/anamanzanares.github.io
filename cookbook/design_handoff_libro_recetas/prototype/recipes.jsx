// Recetas placeholder + arte SVG abstracto por receta + helpers compartidos.
// Todo se cuelga de window para que otros script-babel lo vean.
...
const CATEGORIES = [
  { name: 'Bowls', count: 2 },
  { name: 'Pasta', count: 2 },
  { name: 'Cenas rápidas', count: 3 },
  { name: 'Brunch', count: 1 },
  { name: 'Postres', count: 0 },
];

// Lista de la compra demo
const SHOPPING = {
  fresco: [
    { name: 'Salmón fresco', qty: '220 g', recipe: 'Poke bowl', done: false },
    { name: 'Pechuga de pollo', qty: '300 g', recipe: 'Ñoquis con pollo', done: false },
    { name: 'Calabacín grande', qty: '1', recipe: 'Calabacín airfryer', done: true },
    { name: 'Aguacate maduro', qty: '1', recipe: 'Tostada', done: false },
    { name: 'Mango maduro', qty: '1', recipe: 'Poke bowl', done: false },
    { name: 'Limones', qty: '3', recipe: 'Varias', done: true },
  ],
  despensa: [
    { name: 'Arroz para sushi', qty: '180 g', recipe: 'Poke bowl', done: true },
    { name: 'Ñoquis frescos', qty: '400 g', recipe: 'Ñoquis', done: false },
    { name: 'Leche de coco', qty: '1 lata', recipe: 'Curry rápido', done: false },
    { name: 'Pasta de curry rojo', qty: '1 tarro', recipe: 'Curry rápido', done: false },
  ],
  nevera: [
    { name: 'Yogur griego', qty: '150 g', recipe: 'Ñoquis con pollo', done: false },
    { name: 'Feta', qty: '120 g', recipe: 'Ensalada garbanzos', done: false },
    { name: 'Parmesano', qty: '60 g', recipe: 'Pasta al limón', done: true },
  ],
};

// ----- Componentes compartidos -----

// Marco interno de pantalla: aplica el tema, ocupa el artboard entero,
// y opcionalmente añade densidad. El artboard del canvas trae su propio
// borde gris; el marco lo cubre con el bg del tema.
function Frame({ theme = 'c', density = 'comfortable', children, style, scroll = false, className = '' }) {
  return (
    <div
      data-theme={theme}
...
// Etiqueta tipo "ROUTE 01 / C"
function Eyebrow({ children, as = 'p', style }) {
  const Tag = as;
  return (
    <Tag className="ph-eyebrow" style={style}>{children}</Tag>
  );
}

// Regla horizontal 1px en color accent
...
// Mapa de "tono" → trío (bg / forma / matiz). Le da carácter a cada receta
// sin que las pruebes individualmente.
const PHOTO_TONES = {
  ink:    { bg: 'ink',    shape: 'coral',  tint: 'beige' },
  coral:  { bg: 'coral',  shape: 'beige',  tint: 'ink' },
  beige:  { bg: 'beige',  shape: 'ink',    tint: 'coral' },
  peach:  { bg: 'peach',  shape: 'coral',  tint: 'ink' },
  orange: { bg: 'orange', shape: 'ink',    tint: 'beige' },
  red:    { bg: 'red',    shape: 'beige',  tint: 'ink' },
};

// Arte SVG abstracto (el placeholder bonito) — visible solo cuando la
// receta no tiene foto todavía.
function RecipeArtSvg({ recipe, style }) {
  const t = PHOTO_TONES[recipe.photoTone] || PHOTO_TONES.coral;
  const bg = `var(--color-${t.bg})`;
  const shapeColor = `var(--color-${t.shape})`;
  const tint = `var(--color-${t.tint})`;

  return (
    <div className="ph-art" style={{ background: bg, position: 'absolute', inset: 0, overflow: 'hidden' }}>
      <svg viewBox="0 0 200 200" preserveAspectRatio="xMidYMid slice" width="100%" height="100%" style={{ display: 'block' }}>
        <defs>
          <pattern id={`grain-${recipe.id}`} patternUnits="userSpaceOnUse" width="3" height="3">
            <rect width="3" height="3" fill="transparent" />
            <circle cx="1" cy="1" r="0.4" fill="rgba(0,0,0,0.07)" />
          </pattern>
        </defs>
        <ArtBody photo={recipe.photo} shape={shapeColor} tint={tint} />
        <rect width="200" height="200" fill={`url(#grain-${recipe.id})`} />
      </svg>
    </div>
  );
}

// RecipeArt = SVG placeholder + image-slot encima.
// Mismo id por receta en TODAS las pantallas → arrastra una foto una sola vez
// (por ejemplo en el detalle) y aparece en home, índice, modo cocina, etc.
function RecipeArt({ recipe, style }) {
  return (
    <div className="ph-recipe-photo" style={{ position: 'relative', overflow: 'hidden', ...style }}>
      <RecipeArtSvg recipe={recipe} />
      <image-slot
        id={`recipe-${recipe.id}`}
        shape="rect"
        fit="cover"
...
function ArtBody({ photo, shape, tint }) {
  switch (photo) {
    case 'circle': // poke bowl — un círculo grande con puntos dentro (toppings)
      return (
        <>
          <circle cx="100" cy="105" r="68" fill={shape} />
    ...
    case 'ovals': // ñoquis — óvalos esparcidos
      return (
        <g fill={shape}>
          <ellipse cx="60"  cy="70"  rx="22" ry="14" transform="rotate(-12 60 70)" />
          <ellipse cx="120" cy="60"  rx="22" ry="14" transform="rotate(8 120 60)" />
          <ellipse cx="155" cy="105" rx="22" ry="14" transform="rotate(-22 155 105)" />
          <ellipse cx="80"  cy="130" rx="22" ry="14" transform="rotate(14 80 130)" />
          <ellipse cx="135" cy="150" rx="22" ry="14" transform="rotate(-6 135 150)" />
        </g>
      );
    case 'slices': // calabacín — rodajas alineadas
      return (
        <g>
          <circle cx="55"  cy="100" r="22" fill={shape} />
          <circle cx="55"  cy="100" r="10" fill={tint} />
          <circle cx="100" cy="100" r="22" fill={shape} />
          <circle cx="100" cy="100" r="10" fill={tint} />
          <circle cx="145" cy="100" r="22" fill={shape} />
          <circle cx="145" cy="100" r="10" fill={tint} />
        </g>
      );
    case 'swirl': // pasta — espiral/curva
      return (
        <g fill="none" stroke={shape} strokeWidth="9" strokeLinecap="round">
          <path d="M40 60 C 60 40, 140 40, 160 60 S 160 140, 140 160 S 60 160, 40 140" />
          <path d="M70 90 C 85 80, 115 80, 130 90 S 130 130, 115 140 S 85 140, 70 130" />
        </g>
      );
    case 'stacked': // tostada — rectángulos apilados
      return (
        <g>
          <rect x="30" y="80" width="140" height="40" fill={shape} />
          <rect x="35" y="58" width="130" height="22" fill={tint} />
          <rect x="50" y="46" width="100" height="14" fill={shape} opacity="0.6" />
        </g>
      );
    case 'dots': // ensalada garbanzos — muchos puntos pequeños
      return (
        <g fill={shape}>
          {[[55,55],[90,50],[130,60],[160,75],[60,90],[100,85],[140,95],[170,110],[50,120],[85,115],[120,125],[155,140],[75,150],[110,155],[145,170]].map(([cx, cy], i) => (
            <circle key={i} cx={cx} cy={cy} r="8" />
          ))}
        </g>
      );
    case 'curry': // curry — círculo + lima cuña
      return (
        <>
          <circle cx="95" cy="110" r="68" fill={shape} />
          <circle cx="95" cy="110" r="50" fill={tint} opacity="0.45" />
          <path d="M150 50 L 175 65 L 162 90 Z" fill={shape} />
        </>
      );
    case 'fish': // salmón — elipse con líneas paralelas
      return (
        <g>
          <ellipse cx="100" cy="100" rx="75" ry="35" fill={shape} />
          <g stroke={tint} strokeWidth="3" opacity="0.6">
            <line x1="40" y1="100" x2="160" y2="100" />
            <line x1="50" y1="85"  x2="155" y2="85" />
            <line x1="50" y1="115" x2="155" y2="115" />
          </g>
        </g>
      );
    default:
      return <circle cx="100" cy="100" r="60" fill={shape} />;
  }
}

Object.assign(window, { RECIPES, CATEGORIES, SHOPPING, Frame, Eyebrow, Rule, RecipeArt });
